...
with open ('/home/amin/1_HEMA/1_MySQL/1_ensembl_core_database/1_arabidopsis_thaliana/2_attached_table/3_table_seq_region_position_attrib_value/3_physicochemical properties/14_at_PersistanceLength15_SW_101bp_10018.csv', 'a') as csvfile:
	...
	for seq in SeqIO.parse(fasta_file,"fasta"):
		...
		for i in range(len(seq.seq)):
			if seq.seq[i] != 'N':
				n=n+1
				writer.writerow([n,10018,array[1][c]])
				c=c+1
			else:
				n=n+1
				writer.writerow([n,10018,'Null'])
