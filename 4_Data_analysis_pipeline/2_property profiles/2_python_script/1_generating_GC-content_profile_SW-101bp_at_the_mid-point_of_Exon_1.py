...
for i in range(len(features)):
	...
	#print start,end,seq_region_id
	cur = db.cursor()
	cur.execute("SELECT srpav.value FROM HEMA_arabidopsis_thaliana_core_40_93_11.seq_region_pos_attrib_value srpav where srpav.attrib_analysis_id=10004 and srpav.seq_region_pos_id between %s and %s" % (start, end))
	...
	for j in range(len(transcript_prop_value)):
		...
		if transcript_prop_value[j][0] !=None:
